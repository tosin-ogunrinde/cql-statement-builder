 ...
 */
public class SingleQuoteEscaper {

    private SingleQuoteEscaper() {
    }

    public static String escapeDateOrStringValue(Object value) {
        ObjectValidator.validateNotNull(value);
        if (!(value instanceof Date) && !(value instanceof String)) {
            throw new IllegalArgumentException("value must either be a Date or String: " + value);
        }
        return "'" + value + "'";
    }
}